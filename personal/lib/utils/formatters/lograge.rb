  ...
  module Formatters
    class Lograge
      ...
      def format(data)
        ...
        formatted_hash[:host] = data.delete(:host)
        formatted_hash[:logger] = { name: :lograge }
        formatted_hash[:time] = time
        formatted_hash[:network] = { client: { ip: data.delete(:ip) } }
        formatted_hash[:usr] =
          { id: data.delete(:user_id), name: data.delete(:name), email: data.delete(:email) }.compact
        formatted_hash[:http] =
          { request_id:, version: data.delete(:http_version), status_code:, url: data.delete(:url), useragent: data.delete(:ua),
            method: http_method, url_details: { path: data.delete(:path), params: data.delete(:params) } }
        if data[:exception]
          kind, message = data.delete(:exception)
          formatted_hash[:error] = { kind:, message: }
        end
